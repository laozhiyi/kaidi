...
- 收藏量由云端收藏记录统计，自己的收藏操作成功后立即更新，其他用户的变化在可接单列表和详情页每 5 秒自动同步。离开页面后停止同步，返回页面时重新获取；不向客户端开放收藏者身份或私密订单数据。
...
需要部署完整 `mcloud` 云函数和对应小程序，并配置 `bx_order_feed` 权限、复合索引及每分钟维护触发器。具体规则、索引、发布顺序和真实云压测要求见 [云开发稳定性与流畅度上线说明](RELIABILITY.md)。本地并发模拟不代表已经完成线上部署或通过指定容量认证。
