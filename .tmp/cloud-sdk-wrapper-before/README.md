...
需要配套更新 `mcloud` 与小程序后生效。手机号授权接口保留 `phonenumber.getPhoneNumber` 权限，手填路径不调用该接口。实现、旧账号处理及真机步骤见 [登录与注册说明](deployment/WECHAT_LOGIN.md)。真机日志已定位原生权限拒绝 `102`，与后台预拉取日志不同；手填注册可用于继续测试。本地 535 项回归及微信模板编译通过，本任务尚未部署或完成真机验收。

注册与后台设置的文字检查已修正：按实际文字字段说明拒绝原因；权限或超时等接口故障显示服务问题。仅改数值／开关或提交原结算说明时，后台保存不依赖文字接口。详细行为、错误码和 `mcloud` 上传步骤见 [文字检查修复说明](deployment/CONTENT_AUDIT.md)。
