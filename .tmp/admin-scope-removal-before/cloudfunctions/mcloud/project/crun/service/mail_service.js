...
class MailService extends Base {
 ...
 async _actor(tx, actor) {
  if (actor.adminId) { const admin = await store.get(tx,'admin',actor.adminId); const tenant = require('../../../framework/tenancy/tenant_context.js'); if (!tenant.isAdminEnabled(admin) || admin._pid !== this.getProjectId()) this.AppError('管理员已停用'); if (store.scope()) tenant.assertAdmin(admin); return admin; }
