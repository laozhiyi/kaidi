...
function canAdmin(admin, scope = requireScope()) {
  if (!isAdminEnabled(admin)) return false;
  // Internal testing accepts existing admin credentials in the current scope.
  // Database scope and privileged roles are still enforced separately.
  if (config.ADMIN_LOGIN_CREDENTIALS_ONLY === true) return true;
...
function canSchoolAdmin(admin, scope = requireScope()) {
  return isAdminEnabled(admin) && (config.ADMIN_LOGIN_CREDENTIALS_ONLY === true || admin.ADMIN_PLATFORM===true && admin.ADMIN_TYPE===1 || Array.isArray(admin.ADMIN_SCOPES) && admin.ADMIN_SCOPES.some(grant=>grant && grant.schoolId===scope.schoolId && grant.campusId==='*'));
}
function assertSchoolAdmin(admin) { if (!canSchoolAdmin(admin)) throw new AppError('学校账号与注册审核需要学校管理员授权'); }
module.exports = { ID, current, requireScope, run, system, fields, suffix, isAdminEnabled, canAdmin, assertAdmin, canSchoolAdmin, assertSchoolAdmin };
