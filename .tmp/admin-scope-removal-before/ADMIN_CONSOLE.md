...
后台按业务模块组织，采用独立的小程序页面路由。工作台汇总待办；订单、投诉、用户分别进入列表和详情；营业、价格、履约规则分别维护。底部主入口为 **工作台 / 订单 / 投诉 / 用户 / 管理**。严格模式下学校共享账号的用户入口仅对学校或平台管理员显示；当前内测登录规则见下节。

2026-09-21 多校版本增加 `tenants/admin_tenants` 学校、校区、地点及管理员授权页面，仅平台管理员可使用。目录先加载元数据，选中后再读取地点或授权详情；详情失败时禁止用空表单覆盖。当前内测版已移除管理员登录与管理中心新增的学校／校区选择器 UI，继续使用已保存或默认校区，底层上下文与授权逻辑保留。完整部署及授权模型见 [多校部署手册](deployment/MULTI_TENANT_ROLLOUT.md)。

## 内测登录

按本轮要求，`cloudfunctions/mcloud/config/config.js` 的 `ADMIN_LOGIN_CREDENTIALS_ONLY` 默认开启：已有管理员账号和密码匹配即可登录，暂不检查登录次数、账号停用状态或学校／校区授权；缺少新授权字段的旧账号也能进入当前校区后台。该规则同时应用于会话与后台事务，避免进入后再次被同一限制挡住。

仍校验最新登录令牌、登录微信身份和有效期；普通管理员不能使用超级管理员功能，也不会自动变为平台管理员。数据查询和写入始终受当前学校／校区约束。临时开放不修改数据库中的状态、角色或授权。

重新上传 `mcloud`（云端安装依赖）并重新编译小程序后生效。恢复严格模式时，在云函数设置 `ADMIN_LOGIN_CREDENTIALS_ONLY=false`，先为旧管理员配置正式授权，再重新登录。初始化账号及密码要求不变，不会因提交默认密码自动创建管理员。
