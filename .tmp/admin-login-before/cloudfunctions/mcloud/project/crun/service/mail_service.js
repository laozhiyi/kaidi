...
class MailService extends Base {
 ...
 async _actor(tx, actor) {
  if (actor.adminId) { const admin = await store.get(tx,'admin',actor.adminId); if (!admin || admin._pid !== this.getProjectId() || admin.ADMIN_STATUS !== 1) this.AppError('管理员已停用'); if (store.scope()) require('../../../framework/tenancy/tenant_context.js').assertAdmin(admin); return admin; }
