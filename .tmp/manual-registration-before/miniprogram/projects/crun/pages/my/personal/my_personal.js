...
Page(Object.assign({
  data: { isLoad: false, isEdit: true, loadError: '' },
  async onLoad() {
    ProjectBiz.initPage(this);
    const cached = profileMethods.getCachedProfileUser();
    if (cached && cached.USER_MOBILE_VERIFIED === true && cached.USER_PROFILE_COMPLETE === true) { profileMethods.applyUser(this, cached); this.setData({ isLoad: true, user: cached }); }
  ...
  async _loadDetail() {
    ...
    try {
      ...
      if (!user || user.USER_STATUS !== 9 && (user.USER_MOBILE_VERIFIED !== true || user.USER_PROFILE_COMPLETE !== true)) return wx.redirectTo({ url: '../reg/my_reg' });
