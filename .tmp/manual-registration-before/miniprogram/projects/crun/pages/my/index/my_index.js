...
Page({
  ...
  onLoad() {
    ...
    if (PassportBiz.isLogin()) {
      ...
      this.setData({ user: { USER_NAME: token.name, USER_PIC: token.pic || '', USER_STATUS: token.status, USER_MOBILE_VERIFIED: token.phoneVerified, USER_PROFILE_COMPLETE: token.profileComplete } });
    }
  },
  async onShow() {
    this._visible = true;
  ...
  _loadUser() {
    ...
    this._userRequest = (async () => {
      try {
        ...
        if (user && user.USER_STATUS !== 9 && (user.USER_MOBILE_VERIFIED !== true || user.USER_PROFILE_COMPLETE !== true)) {
