...
function harness(options = {}) {
  ...
  const responses = {
    'operations/config': { campuses: ['育才校区', '王城校区', '雁山校区'], locations:require('../../cloudfunctions/mcloud/project/crun/service/tenant_defaults.js').locations },
  ...
  let passport = {
    ...
    isLogin: () => !!(token && token.status === 1 && token.phoneVerified && token.profileComplete), setToken: value => { token = value; }, clearToken: () => { token = null; },
