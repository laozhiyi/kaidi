...
function smokeRender(code) {
  ...
  for (const fixture of loginCases) {
    ...
    if (fixture.profile) {
      ...
      assert.ok(inputs.some(node => node.attr.type === 'nickname'), '昵称应支持微信昵称填写');
      assert.ok(!inputs.some(node => node.attr.bindinput === 'bindProfileMobileInput' || node.attr.value === data.formMobile), '已验证手机号不能手填替换');
      assert.ok(controls.some(button => button.attr.openType === 'chooseAvatar'), '头像应支持微信选择及修改');
      const phone = controls.find(button => button.attr.bindgetphonenumber === 'bindWechatPhone');
      assert.equal(phone && phone.attr.openType, 'getPhoneNumber');
  ...
  console.log('WXML 渲染冒烟检查通过：' + (scenarios.length + receiptCases.length + mailCases.length + packageCases.length + reputationCases.length + adminCases.length + notificationCases.length + loginCases.length) + ' 个页面状态（含微信授权、强制补全资料、收货入口、公告未读、后台表单与隐私检查）。');
}
