...
## 后台管理系统截图 
- 内测后台已改为已有账号密码正确即可登录，暂不受登录次数、管理员状态和校区授权限制；会话与后台事务采用同一规则，角色和当前学校／校区数据隔离仍生效。同步上传 `mcloud` 并重新编译小程序后生效；服务端设置 `ADMIN_LOGIN_CREDENTIALS_ONLY=false` 可恢复严格模式，详见 [后台说明](ADMIN_CONSOLE.md#内测登录)。
- 首次空库安装通过环境变量 `INIT_ADMIN_NAME`（默认 `admin`）和至少 12 位的 `INIT_ADMIN_PASSWORD` 创建平台管理员。没有默认密码；恢复严格模式前旧库管理员需按多校迁移计划明确授权，初始化后移除初始密码变量。
...
AppID 为 `wx3d8dc6fb0e764ec7`。点击登录后先通过微信原生按钮授权手机号，再补全昵称、头像、性别、学院、专业与所在校区。昵称和头像可修改；手机号只接受云端验证的微信授权结果，更换号码须重新授权。未完成验证或资料补全的账号，前端入口与云端订单服务都会拦截；学校审核和账号停用规则继续生效。

需要配套更新 `mcloud` 与小程序，云函数已增加 `phonenumber.getPhoneNumber` 权限。实现、旧账号处理及真机步骤见 [微信登录说明](deployment/WECHAT_LOGIN.md)。针对真机点击只转圈的反馈，已增加原生授权／云请求阶段日志和明确错误提示；本地 452 项回归及微信模板编译通过，本任务尚未部署，真实失败原因仍需结合授权回调和云端日志确认。
