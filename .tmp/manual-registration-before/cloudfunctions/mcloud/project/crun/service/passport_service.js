...
class PassportService extends BaseProjectService {
 ...
 async _saveProfile(userId, input, registering) {
  ...
  if (!previous || previous.USER_MOBILE_VERIFIED !== true) this.AppError('请先使用微信手机号授权登录');
  const name = rules.text(input.name, '昵称', 30, true), mobile = rules.text(input.mobile, '手机', 11, true);
  if (mobile !== previous.USER_MOBILE) this.AppError('手机号已变化，请重新授权微信手机号后保存');
  ...
  if (!cfg.campuses.includes(campus) && campus !== profileRules.formValue(previous.USER_FORMS, 'campus')) this.AppError('请选择有效的所在校区');
  ...
  await store.transaction(async tx => {
   ...
   if (current.USER_STATUS === 8) current.USER_STATUS = 0;
   else if (current.USER_PROFILE_COMPLETE === false) current.USER_STATUS = registrationReview ? 0 : 1;
   const now = Date.now();
   Object.assign(current, { USER_NAME: name, USER_PIC: pic, USER_FORMS: forms, USER_OBJ: dataUtil.dbForms2Obj(forms), USER_PROFILE_COMPLETE: true, USER_EDIT_TIME: now });
 ...
 async wechatLogin(userId, input) {
  ...
  await store.transaction(async tx => {
   ...
   const now = Date.now();
   const data = current || { _pid: this.getProjectId(), USER_ID: 'USER' + id.slice(0, 28), USER_MINI_OPENID: userId,
    USER_STATUS: 0, USER_PROFILE_COMPLETE: false, USER_NAME: '', USER_PIC: '', USER_FORMS: [], USER_OBJ: {}, USER_LOGIN_CNT: 0, USER_ADD_TIME: now };
  ...
  return user ? { ...user, USER_MOBILE_VERIFIED: user.USER_MOBILE_VERIFIED === true, USER_PROFILE_COMPLETE: profileRules.isComplete(user) } : null;
	}
