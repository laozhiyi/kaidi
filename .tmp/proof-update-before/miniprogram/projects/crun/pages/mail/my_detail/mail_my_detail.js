...
Page({
  ...
  try {
   ...
   this.setData({ mail, detailUI: MailUI.detail(mail), loading: false }, () => {
    ...
    if (this._confirmAfterLoad) {
     ...
     if (this.data.detailUI.primary === 'deliver') this.bindPanel({ currentTarget: { dataset: { action: 'deliver' } } });
 ...
 async perform(action) {
  ...
  const allowed = ['pickup', 'deliver', 'confirm'].includes(action) && ui.primary === action || action === 'exception' && ui.canException || action === 'cancel' && ui.canCancel;
  if (!allowed) return;
  ...
  try {
   ...
   const route = { pickup: 'mail/pickup', deliver: 'mail/deliver', exception: 'mail/exception', confirm: 'mail/finish', cancel: 'mail/cancel' }[action];
