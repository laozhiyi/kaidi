...
function pageState(fixture) {
 if (!definitions.has(fixture.base)) {
  let definition;
  runMiniProgram(path.join(mini, fixture.base + '.js'), { Page: value => { definition = value; }, Component: value => { definition = value; }, require: name => name.includes('mail_ui_biz') ? UI : {} });
  definitions.set(fixture.base, definition);
 }
 const definition = definitions.get(fixture.base);
 const state = { ...definition.data, ...fixture.data };
 if (definition._decorateOrder && state.dataList) state.dataList = { ...state.dataList, list: state.dataList.list.map(row => definition._decorateOrder(row)) };
 return state;
}
...
function scenarios() {
 ...
 const available = { ...accepted, myaccept: false, MAIL_STATUS: 0, MAIL_END_TIME: Date.now() + 86400000, canAccept: true, canSeeCode: false,
  MAIL_OBJ: { small: 2, price: 4, address1: accepted.MAIL_OBJ.address1, address2: accepted.MAIL_OBJ.address2 } };
 const publicLocations = ['二期 · 中通', '五期 · 邮政', '一期1号宿舍楼201室'];
 const privateContent = ['小林', '13800000000', 'A-314', 'B-628', '订单动态', '接单后可查看详细地址'];
 const publishedAddress = require('../../cloudfunctions/mcloud/project/crun/service/order_rules.js').project({
  ...available, MAIL_OBJ: { ...available.MAIL_OBJ, campus: '雁山校区', address2: '47' },
  MAIL_FORMS: [{ mark: 'campus', val: '雁山校区' }, { mark: 'address2', val: '四期 64' }]
 }, '');
 const addressChecks = { present: ['雁山校区 · 四期 64'], absent: ['雁山校区 · 47', '期数待补充', ...privateContent] };
 return [
  ...
  { id: 'pickup-detail', title: '接单详情 · 取件点与凭证逐件对应', base: 'projects/crun/pages/mail/my_detail/mail_my_detail', data: { mail: accepted, detailUI: UI.detail(accepted), loading: false }, expected: '取件凭证', present: ['第1件 · 小件', '第2件 · 小件', ...publicLocations, 'A-314', 'B-628'], absent: ['订单动态'] },
  { id: 'pickup-available-list', title: '可接单列表 · 显示全部取件点和送达地点', base: 'projects/crun/pages/order/index/order_index', data: { isLoad: true, tabIndex: 0, dataList: { list: [available] } }, expected: '可接单', present: publicLocations, absent: privateContent },
  { id: 'pickup-public-detail', title: '未接单详情 · 显示全部取件点和送达地点', base: 'projects/crun/pages/mail/detail/mail_detail', data: { isLoad: true, mail: available, detailUI: UI.detail(available), loading: false }, expected: '送达地点', present: publicLocations, absent: privateContent },
  { id: 'published-address-list', title: '送达 · 使用发布订单的四期64地址', base: 'projects/crun/pages/order/index/order_index', data: { isLoad: true, tabIndex: 0, dataList: { list: [publishedAddress] } }, expected: '送达', ...addressChecks },
  { id: 'published-address-detail', title: '送达地点 · 使用发布订单的四期64地址', base: 'projects/crun/pages/mail/detail/mail_detail', data: { isLoad: true, mail: publishedAddress, detailUI: UI.detail(publishedAddress) }, expected: '送达地点', ...addressChecks }
 ];
}
