...
function mail(status = 0, role = 'poster') {
 return { _id: '68bc410aa6b530bd14ff9831', MAIL_ID: 'KD20260907102816001', MAIL_STATUS: status, MAIL_PAYMENT_MODE: 'offline', MAIL_TOTAL_FEE: 600, MAIL_ADD_TIME: now - 600000,
  ...
  MAIL_ACCEPT_TIME: [1,4,2,3,9].includes(status) ? now - 300000 : 0, MAIL_PICKUP_TIME: [4,2,9].includes(status) ? now - 120000 : 0, MAIL_OVER_TIME: status === 9 ? now + 60000 : 0,
...
function detailFixture(name, status, role, patch = {}, progressExpected) {
 ...
 const serviceType = order.MAIL_OBJ.serviceType || 'take';
 const stage = status === 0 && order.MAIL_END_TIME <= now ? -1 : ({ 0: 0, 1: 1, 4: 2, 2: 3, 9: 4 }[status] ?? -1);
 const labels = ['已发布', '已接单', serviceType === 'buy' ? '已购齐' : '已取件', '已送达', '已完成'];
 const pending = ['待发布', '待接单', serviceType === 'buy' ? '待购买' : '待取件', '待送达', '待完成'];
 return { name, page: role === 'public' ? 'detail/mail_detail' : 'my_detail/mail_my_detail', expected: detailUI.title,
  present: [order.MAIL_OBJ.address1,order.MAIL_OBJ.campus + ' · ' + order.MAIL_OBJ.address2,...(role === 'public' ? [] : ['订单信息',order.MAIL_ID,...(serviceType === 'take' ? ['取件凭证', ...order.MAIL_OBJ.code.split('\n')] : serviceType === 'send' ? ['物品与交接信息'] : []),...(role === 'poster' ? ['订单动态'] : [])])],
  ...
  progressExpected: progressExpected || { labels: stage < 0 ? [] : labels.map((label, index) => index <= stage ? label : pending[index]), done: stage < 0 ? [] : labels.map((_, index) => index <= stage), current: stage },
...
function scenarios() {
 ...
 const fixtures = [
  ...
  { name: 'publish-late-unrestricted', page: 'add/mail_add', data: { ...publish, serviceState: UI.service(config, Date.parse('2026-09-07T23:00:00+08:00')) }, expected: '全天可发布', present: ['发布代取订单'], absent: ['休息中，营业后可发布', '暂时停止接单'] },
  { name: 'publish-late-hours', page: 'add/mail_add', data: { ...publish, config: { ...config, enforceBusinessHours: true }, serviceState: UI.service({ ...config, enforceBusinessHours: true }, Date.parse('2026-09-07T23:00:00+08:00')) }, expected: '休息中，营业后可发布', absent: ['正常接单'] },
  { name: 'publish-error', page: 'add/mail_add', data: { isLoad: false, configError: true, loadError: '网络暂不可用，请检查连接后重试' }, expected: '重新加载' },
  { name: 'publish-stations', page: 'add/mail_add', data: { ...publish, packagePickupVisible: true, packagePickupNumber: 1, pickupStations: require('../../cloudfunctions/mcloud/project/crun/service/tenant_defaults.js').locations.pickupStations }, expected: '选择常用快递点', present: ['中通','邮政','京东','其他取件点','确认取件点'], absent: ['从哪里取，送到哪里','取件快递点'] },
 ...
 fixtures.push(detailFixture('detail-admin-completed', 9, 'poster', {
  MAIL_ACCEPT_TIME: 0, MAIL_PICKUP_TIME: 0, MAIL_DELIVERED_TIME: 0, MAIL_DELIVERY_PROOF: null,
  MAIL_EXCEPTION: { resolution: 'complete', note: '双方同意人工完结' },
  MAIL_HISTORY: [{ action: 'publish', at: now - 600000 }, { action: 'resolve', status: 9, actor: 'admin', at: now + 60000 }]
 }, { labels: ['已发布', '接单', '取件', '送达', '已完结'], done: [true, false, false, false, true], current: 4, unrecorded: [1, 2, 3] }));
 fixtures.push(detailFixture('detail-legacy-delivered', 2, 'rider', { MAIL_ACCEPT_TIME: 0, MAIL_PICKUP_TIME: 0 },
  { labels: ['已发布', '接单', '取件', '已送达', '待完成'], done: [true, false, false, true, false], current: 3, unrecorded: [1, 2] }));
 for (const serviceType of ['buy', 'send']) for (const role of ['poster', 'rider', 'public']) {
  const fixture = detailFixture('detail-' + serviceType + '-' + role, 2, role, { MAIL_OBJ: { ...obj, serviceType, goods: '矿泉水', buyQuantity: 2, goodsBudget: 6 } });
  fixture.sectionCounts = { '购买要求': serviceType === 'buy' ? 1 : 0, '订单信息': role === 'public' ? 0 : 1, '取件凭证': 0, '物品与交接信息': serviceType === 'send' && role !== 'public' ? 1 : 0 };
