...
test('order transactions ignore school grants and still recheck live admin status',async()=>{
  const f=await setup(),id=(await publish(f,A))._id,Mail=f.service('mail_service');
  f.table('admin').set('local',{_id:'local',_pid:'crun',ADMIN_STATUS:0,ADMIN_TYPE:1,ADMIN_SCOPES:[{schoolId:A.schoolId,campusId:B.campusId}]});
  await assert.rejects(tenant.run(A,()=>new Mail().holdMail('local',id,{requestId:req('hold'),note:'核验'})),/停用/);
  assert.equal(f.table('mail').get(id).MAIL_STATUS,0);
  f.table('admin').get('local').ADMIN_STATUS=1;
  await tenant.run(A,()=>new Mail().holdMail('local',id,{requestId:req('hold'),note:'核验'}));
  assert.equal(f.table('mail').get(id).MAIL_STATUS,3);
  assert.equal(f.table('admin').get('local').ADMIN_SCOPES[0].campusId,B.campusId);
});
...
test('business hours controls only affect the active campus without requiring campus grants',async()=>{
  ...
  await tenant.run(B,()=>new Config().saveConfig({enforceBusinessHours:true},local._id,'service'));
  assert.equal((await tenant.run(B,()=>new Config().getConfig())).enforceBusinessHours,true);
  assert.equal((await tenant.run(C,()=>new Config().getConfig())).enforceBusinessHours,false);
});
...
test('super admins can update the shared school registration policy without school grants',async()=>{
  ...
  await tenant.run(A,async()=>{
    await new (f.service('operation_config_service'))().saveConfig({registrationReview:false},local._id,'rules');
  });
  assert.equal(f.table('school').get(A.schoolId).registrationReview,false);
  const resolved=await new (f.service('tenant_service'))().resolve(B);
  assert.equal((await tenant.run(resolved,()=>new (f.service('operation_config_service'))().getConfig())).registrationReview,false);
  assert.equal((await tenant.run(B,()=>new Passport().login('new-user'))).token.status,0,'changing registration policy does not approve existing pending users');
});
