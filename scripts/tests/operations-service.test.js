...
test('registration is unique under racing calls and cannot self-authorize or edit a disabled profile',async()=>{
 const f=fixture(),svc=new (f.load('passport_service.js'))();f.config.registrationReview=true;const input={name:'同学',mobile:'13912345678',pic:'cloud://test/avatar.png',forms:[{mark:'sex',val:'男'},{mark:'college',val:'计算机学院'},{mark:'sub',val:'软件工程'},{mark:'campus',val:'育才校区'}],status:1};
 await svc.wechatIdentityLogin('newuser');
 ...
 await svc.wechatIdentityLogin('newuser2');
