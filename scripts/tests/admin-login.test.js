...
test('internal testing defaults to credentials-only login and can explicitly restore account checks', () => {
...
test('restoring account checks does not restore school grants but still rejects disabled accounts and rate limits', async () => {
  const f = await adminFixture({ credentialsOnly: false, ADMIN_NAME: 'admin' });
  const session = await f.login();
  assert.equal(session.credentialsOnly, false);
  assert.equal(f.table('admin').get('legacy').ADMIN_SCOPES, undefined);
...
for (const credentialsOnly of [true, false]) {
  test('real login, dashboard and user routes ignore missing and foreign grants: credentialsOnly=' + credentialsOnly, async () => {
    const f = await adminFixture({ credentialsOnly, ADMIN_TYPE: 0,
      ADMIN_SCOPES: [{ schoolId: 'unrelated_school', campusId: 'other' }] });
    const app = f.load('framework/core/application.js');
    const loggedIn = await app.app({ route: 'admin/login', PID: 'crun', scope: A, params: { name: 'ops', pwd: secret } }, {});
    assert.equal(loggedIn.code, 200, loggedIn.msg);
    const token = loggedIn.data.token;
    delete f.table('admin').get('legacy').ADMIN_SCOPES;
    for (const scope of [A, B, f.C]) {
      const home = await app.app({ route: 'admin/home', PID: 'crun', scope, token, params: {} }, {});
      assert.equal(home.code, 200, home.msg);
      const student = [...f.table('user').values()].find(row => row.schoolId === scope.schoolId && row.USER_MINI_OPENID === 'poster');
      const user = await app.app({ route: 'admin/user_detail', PID: 'crun', scope, token, params: { id: student._id } }, {});
      assert.equal(user.code, 200, user.msg);
      assert.equal(user.data.USER_NAME, 'poster');
    }
    const invalid = await app.app({ route: 'admin/home', PID: 'crun', scope: A, token: 'invalid-token', params: {} }, {});
    assert.notEqual(invalid.code, 200);
    await assert.rejects(tenant.run(A, () => f.sessions.isSuperAdmin(token, 'poster')));
  });
}
...
for (const credentialsOnly of [undefined, false, true]) {
test('admin client exposes user management without school grants: credentialsOnly=' + credentialsOnly, async () => {
  const session = { token: 'server-token', name: 'ops', type: 0, credentialsOnly, scopes: [] };
  const f = client({ data: session });
  assert.equal(f.biz.isSchoolAdmin(), false);
