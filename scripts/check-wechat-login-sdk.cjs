'use strict';
// Exercise the production cloud wrapper and installed SDK's OpenAPI conversion.
...
process.env.CLOUD_ENV_ID = 'wechat-login-local-only';
...
(async () => {
  try {
    ...
    console.log('Production cloud wrapper + installed wx-server-sdk ' + version + ': phonenumber.getPhoneNumber, code serialization, phoneInfo conversion and invalid-code rejection OK; 2 mocked requests, no network.');
