...
后台按业务模块组织，采用独立的小程序页面路由。工作台汇总待办；订单、投诉、用户分别进入列表和详情；营业、价格、履约规则分别维护。底部主入口为 **工作台 / 订单 / 投诉 / 用户 / 管理**。已登录管理员可进入用户管理，入口不再按学校授权隐藏。

2026-09-21 多校版本增加 `tenants/admin_tenants` 学校、校区、地点及管理员资料页面，仅平台管理员可使用。目录先加载元数据，选中后再读取地点或历史授权详情；详情失败时禁止用空表单覆盖。当前内测版已移除管理员登录与管理中心新增的学校／校区选择器 UI，继续使用已保存或默认校区，底层上下文用于确定数据范围。完整部署说明见 [多校部署手册](deployment/MULTI_TENANT_ROLLOUT.md)。

## 内测登录

2026-09-22 按用户要求直接删除学校／校区管理授权校验，覆盖登录、会话、订单事务、后台设置与用户管理。账号缺少 `ADMIN_SCOPES`，或历史授权指向其他学校／校区，均不再阻挡后台使用。`ADMIN_LOGIN_CREDENTIALS_ONLY=false` 也不会恢复这一限制。

`cloudfunctions/mcloud/config/config.js` 的 `ADMIN_LOGIN_CREDENTIALS_ONLY` 仍默认开启：已有管理员账号和密码匹配即可登录，暂不检查登录次数或账号停用状态。显式设为 false 仅恢复账号状态与登录限流检查。

仍校验最新登录令牌、登录微信身份和有效期；普通管理员不能使用超级管理员功能，也不会自动变为平台管理员。数据查询和写入始终受当前学校／校区约束。临时开放不修改数据库中的状态、角色或授权。

这条权限报错来自云函数，修改本地代码或只重新编译小程序不会覆盖云端版本。请在微信开发者工具确认目标云环境，右键 `cloudfunctions/mcloud`，选择“上传并部署：云端安装依赖”，等待成功后重新编译小程序并登录。初始化账号及密码要求不变，不会因提交默认密码自动创建管理员。本次未执行云端上传或真机验收。

## 文字检查与设置保存

2026-09-22 修复注册和后台保存共用的文字检查错误分类。服务权限、超时和异常回包会明确显示为审核服务问题。真实拒绝会指出“结算说明”等字段，页面保留修改，可调整后再次保存。

营业／履约开关、时间、数量和价格数字不送文字审核；价格页附带的未修改结算说明也不重复送审。服务端仍在保存前验证分组、类型和数值范围，新说明审核通过后才写入配置与操作记录。权限配置及部署步骤见 [文字检查修复说明](deployment/CONTENT_AUDIT.md)。

后续发现共用云 SDK 封装会把 OpenAPI 入口错误绑定，导致文字服务方法无法调用；现已修复。请上传含最新 `framework/cloud/cloud_base.js` 的 `mcloud`，仅上传上一轮错误提示修改不包含这项修复。
...
| 服务设置（新增） | `settings/service/admin_service_settings` | 服务总开关、营业时间限制开关、营业时段、当前校区 |
...
- “按营业时间限制发布与接单”默认关闭，内测可全天发布与接单；旧配置缺少 `enforceBusinessHours` 也按关闭处理。显式开启后按北京时间执行营业时段，前端发布按钮与云端校验一致。“开放发布与接单”总开关仍然生效，已有订单可继续履约；开关保存需要超级管理员权限。
- 订单进度使用发布、接单、取件／购齐、送达、完成五个节点，按实际时间和履约事件显示。管理员提前完结不会补齐未发生的中间步骤；旧记录缺少时间时明确说明。
...
修改 `cloudfunctions/mcloud` 的查询、配置或报表服务后需要重新部署该云函数；客户端修改需要重新编译/发布小程序。自动化服务测试使用本地数据库或 SDK 传输替身，实际云环境的鉴权、文件访问和 Excel 生成仍需部署后联调。测试数量和编译结果以当前源码运行上述命令的输出为准。
