...
Page({
	...
	_loadDetail: async function () {
		...
		if (!this.data.isLoad) this.setData(ThingBiz.initFormData(id)); // 初始化表单数据
		...
		let thing = await cloudHelper.callCloudData('thing/detail', params, opt);
		if (!thing) {
			this.setData({
				isLoad: null
			})
		...
		this.setData({
			...
			formCateId: thing.THING_CATE_ID,
			formOrder: thing.THING_ORDER,
			formEnd: thing.THING_END_TIME,

			formForms: thing.THING_FORMS,
	...
	*/
	...
	bindFormSubmit: async function () {
		...
		try {
			...
			// 创建
			await cloudHelper.callCloudSumbit('thing/edit', data).then(async res => {
				// 图片
				await cloudHelper.transFormsTempPics(forms, 'thing/', id, 'thing/update_forms');

let cb = () => {
				// 更新列表页面数据
				let node = {
					'status': res.data.statusDesc,
					'THING_CATE_ID': data.cateId,
					'end': data.formEnd,
					'THING_OBJ': {
