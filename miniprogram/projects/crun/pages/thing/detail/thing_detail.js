...
Page({
	...
	bindAcceptTap: async function (e) {
		...
		let cb = async () => {
			try {
				...
				await cloudHelper.callCloudSumbit('thing/accept', params, {}).then(res => {
					let cb = () => {
						wx.redirectTo({
							url: 'thing_detail?id=' + this.data.id,
						});
					}
	...
	bindCancelTap: async function (e) {
		...
		let cb = async () => {
			try {
				...
				await cloudHelper.callCloudSumbit('thing/cancel', params, {}).then(res => {
					let callback = () => {
						wx.redirectTo({
							url: 'thing_detail?id=' + this.data.id,
						});
	...
	bindOverTap: async function (e) {
		...
		let cb = async () => {
			try {
				...
				await cloudHelper.callCloudSumbit('thing/status', params, {}).then(res => {
					let callback = () => {
						wx.redirectTo({
							url: 'thing_detail?id=' + this.data.id,
						});
	...
	onShareAppMessage: function (res) {
		return {
			title: this.data.thing.THING_TITLE,
			imageUrl: this.data.thing.THING_PIC[0]
		}
	},
  ...
  */
	getLeftTime() {
		...
		let end_time = this.data.thing.end;
	...
	bindShowPayPicTap: function (e) {
		if (!this.data.thing || !this.data.thing.THING_ACCEPT_PAY_PIC) return;
		wx.previewImage({
			urls: [this.data.thing.THING_ACCEPT_PAY_PIC],
			current: this.data.thing.THING_ACCEPT_PAY_PIC
		});
	},
})