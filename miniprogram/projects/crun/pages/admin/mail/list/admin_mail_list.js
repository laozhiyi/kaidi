...
const JobBiz = require('../../../../biz/mail_biz.js');
...
Page({
	...
	_setSort: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/mail_sort', params).then(res => {
				pageHelper.modifyListNode(id, this.data.dataList.list, 'MAIL_ORDER', sort);
				this.setData({
	...
	_del: async function (e) {
		...
		let callback = async () => {
			try {
				...
				await cloudHelper.callCloudSumbit('admin/mail_del', params, opts).then(res => {
	...
	bindMoreTap: async function (e) {
		...
		let order = this.data.dataList.list[idx].MAIL_ORDER;
	...
	_setStatus: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/mail_status', params).then(res => {
	...
	_getSearchMenu: function () {
		...
		let sortMenus = [
			...
			{ label: '最早ˇ', type: 'sort', value: 'MAIL_ADD_TIME|asc' },
			{ label: '浏览数ˇ', type: 'sort', value: 'MAIL_VIEW_CNT|desc' },
			{ label: '收藏数ˇ', type: 'sort', value: 'MAIL_FAV_CNT|desc' },
