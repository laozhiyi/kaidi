...
Page({
	...
	_loadDetail: async function (isDel) {
		...
		let data = await cloudHelper.callCloudData('admin/mail_data_get', params, options);
	...
	bindOpenTap: function (e) {
		fileHelper.openDoc('快递代取记录', this.data.url);
	},
	...
	bindExportTap: async function (e) {
		...
		try {
			...
			await cloudHelper.callCloudData('admin/mail_data_export', params, options).then(res => {
				this._loadDetail(0);
	...
	bindDelTap: async function (e) {
		try {
			...
			await cloudHelper.callCloudData('admin/mail_data_del', {}, options).then(res => {
				this.setData({
					url: '',
				});
