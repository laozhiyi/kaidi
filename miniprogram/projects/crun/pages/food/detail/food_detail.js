...
Page({
	...
	_loadDetail: async function () {
		...
		let food = await cloudHelper.callCloudData('food/view', params, opt);
		if (!food) {
			this.setData({
				isLoad: null
			})
	...
	bindAcceptTap: async function (e) {
		...
		let cb = async () => {
			try {
				...
				await cloudHelper.callCloudSumbit('food/accept', params, {}).then(res => {
					let cb = () => {
						wx.redirectTo({
	...
	bindCancelTap: async function (e) {
		...
		let cb = async () => {
			try {
				...
				await cloudHelper.callCloudSumbit('food/cancel', params, {}).then(res => {
					let callback = () => {
						wx.redirectTo({
							url: 'food_detail?id=' + this.data.id,
						});
	...
	onShareAppMessage: function (res) {
		return {
			title: this.data.food.FOOD_TITLE,
			imageUrl: this.data.food.FOOD_PIC[0]
		}
	},
  ...
  */
	getLeftTime() {
		...
		let end_time = this.data.food.end;
