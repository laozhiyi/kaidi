...
class ThingController extends BaseProjectController { 
	...
	async statusThing() {
		...
		let rules = {
			...
			pickupPic: 'array|name=取件照片',
			deliverPic: 'array|name=接收照片',
			overDesc: 'string|name=补充说明',
			overDescPic: 'array|name=补充说明照片',
		};

		// 取得数据
		let input = this.validateData(rules);

		// 内容审核（仅当status=9且有补充说明文本时）
		if (Number(input.status) === 9 && input.overDesc) {
			await contentCheck.checkTextMultiClient({ overDesc: input.overDesc });
		}

		let service = new ThingService();
		let ext = {
			pickupPic: input.pickupPic,
			deliverPic: input.deliverPic,
			overDesc: input.overDesc,
			overDescPic: input.overDescPic,
		};
		return await service.statusThing(this._userId, input.id, input.status, input.overTime, ext);
