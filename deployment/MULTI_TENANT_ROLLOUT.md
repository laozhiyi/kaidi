...
| 账户生命周期 | `bx_account_session`、`bx_account_media`、`bx_account_cleanup_file` | 按本应用可信微信身份维护会话、两小时注销及可重试文件清理；资料和正常业务查询仍按学校／校区隔离 |
...
登录默认关闭 `WECHAT_PHONE_LOGIN_ENABLED`。点击“点击微信登录”打开同一头像／昵称弹层，用原生 `chooseAvatar` 和 `nickname` 控件填写；确认后使用可信微信身份建立会话、保存头像昵称，并进入个人资料页。联系资料始终展开，保存时仍执行必填、学校内号码唯一性及审核规则。进入页面或静默刷新不能登录，也不能撤销注销。只有明确恢复旧手机号流程时才设置 `WECHAT_PHONE_LOGIN_ENABLED=true`。详见 [登录与账户说明](WECHAT_LOGIN.md)。

退出会撤销服务端凭证并清空本地个人状态；离线时先退出本机，重启／回到前台后补发原凭证撤销。申请注销时在事务内检查全部学校的未完成发单／接单，成功立即退出；仅主动登录且服务端时间早于截止时间才能撤销注销。等于截止时间已到期。新 `ordersWorker` 每分钟执行到期清理，存储／数据库失败继续重试；处理完成前禁止恢复旧账户。学校／校区停用不阻止退出或注销。
...
云存储权限另行配置：保留当前用户向 `private/<openid>/` 上传的权限，新头像和原图路径含服务端返回的 `mediaGeneration`；证据归档为 `private-evidence/<openid>/<mediaGeneration>/`，禁止客户端写入 `private-evidence/`。私密凭证通过云函数检查订单关系后获取临时链接。重新注册会换目录，迟到的旧注销清理不会删除新账户的同内容文件。数据库规则不能直接用作整个存储桶的策略。当前线下结算；`payNotify` 已关闭，不应配置为支付成功入口。

注销只自动删除目标环境内、有本人路径归属且已登记或仍被资料／业务记录引用的私有文件。历史 `crun/user/...` 共享头像路径缺少可信归属，仅移除数据库引用；上线前须按存储端所有者元数据核对旧文件，不能按用户提交的任意 URL 批量删除。客户端上传后从未交给服务端的孤立原图不在数据库清单中，应另设经过核对的存储清理策略。
...
账户注销新增 `_pid, status, cancelAt` 到期扫描和各关联集合的全局身份索引，以及 `_pid, orderId, accountCleanupId` 事件副本索引。`bx_user`／`bx_identity_unique` 的 `system` 索引仅限标记为 `account_lifecycle` 的跨校清理，日常查询仍用学校索引。三个账户集合均应用 server-only 规则；清理日志不能配置自动 TTL 删除。
...
通知函数单独配置 `ORDER_SUBSCRIBE_TEMPLATE_ID`、`ORDER_SUBSCRIBE_FIELDS`，并授予 `subscribeMessage.send`。字段映射形如 `{"order":"character_string1","status":"thing2","time":"time3"}`，必须使用实际微信模板字段。`mcloud` 保留图片/文字审核、小程序码等原有 OpenAPI 权限；`phonenumber.getPhoneNumber` 声明供保留的旧手机号流程使用，默认的微信身份登录不调用它。不要把所有权限照搬给迁移函数。登录配置及真机步骤见 [微信登录说明](WECHAT_LOGIN.md)。
...
检查成功后，使用控制台/开发者工具“云端安装依赖”部署到确认的环境；API 单独部署 `cloudfunctions/mcloud`。这些本地脚本不会安装、上传或部署。修改源文件后重新生成整个批次，不混用不同批次的函数。账户会话／注销维护同时涉及 API、订单 worker 和小程序，必须配套更新。旧客户端没有新凭证会被拒绝访问私有接口；旧缓存须由用户明确微信登录换取新凭证，不能兼容为仅凭 OPENID 自动恢复。
...
1. 在独立测试环境恢复脱敏备份，确认环境 ID。初始化会补齐集合；版本标记为 `bx_setup_crun_20260923_accounts`，只有包括三个账户集合在内的所有集合成功就绪后才创建，不要手工提前建标记。仅更新本次账户功能不需要清空用户资料或重做已完成的学校迁移。
...
| 微信身份登录与退出 | 默认无手机号授权；退出后保持未登录，打开登录页不恢复账号，再次点击微信登录才恢复原资料；新账号不因登录获得业务资格 |
| 联系资料与保留的手机号流程 | 基础模式手填联系资料，修改已验证号码会清除验证标记；审核与学校内号码归属继续生效。旧手机号流程只在显式开启后验收服务权限与授权 |
