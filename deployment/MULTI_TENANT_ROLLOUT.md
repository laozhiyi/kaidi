...
本手册适用于当前学校／校区架构，取代旧文档中仅按 `_pid` 隔离、客户端监听数据库和 `mcloud` 共用定时器的部署步骤。功能入口与本地检查见 [README](../README.md)。

当前交付是代码、本地检查与可部署材料。尚未连接独立测试云环境，未上传函数、创建云端索引、改变线上安全规则或迁移真实数据。下述云端步骤必须在明确目标环境后执行并留存结果。

注册／后台保存的文字审核修复也需要重新上传 `mcloud`：纯数值／开关与未改文字不调用审核；修改文字按字段检查，服务异常独立提示。见 [文字检查修复说明](CONTENT_AUDIT.md)，本地 worker 快照不代替 API 云函数更新。

云 SDK OpenAPI 封装修复在 `framework/cloud/cloud_base.js`；实际 SDK 的审核、手机号和通知检查均经过生产云封装。部署前按第 3 节从当前源码重新生成并校验整批 worker，旧的本地快照不代表当前版本。

## 1. 数据与权限边界

| 范围 | 集合/内容 | 约束 |
| --- | --- | --- |
| 学校 | `bx_user`、`bx_identity_unique` | 同校跨校区共享账号、手机号唯一性；不同学校独立。用户审核、停用、导出及注册审核政策在当前学校范围执行 |
...
后台学校／校区管理授权校验已按用户要求直接删除，登录、会话、订单事务、配置与用户管理均不再依赖 `ADMIN_SCOPES`。历史字段和迁移格式保留作兼容数据。平台身份仍需同时满足 `ADMIN_TYPE=1`、`ADMIN_PLATFORM=true`；`ADMIN_TYPE=1` 控制高级配置权限，本身不等于平台身份。通过目录页更新管理员角色和历史授权会使目标账号旧令牌失效。

默认启用 `ADMIN_LOGIN_CREDENTIALS_ONLY=true`：内测已有管理员凭正确账号密码即可登录，暂停登录限流与账号状态检查，会话和业务事务同步采用该规则。显式设置 false 仅恢复限流和状态检查，不会恢复已经删除的学校／校区授权门槛。角色、令牌校验及数据库范围隔离继续生效；开关由服务端环境控制，不能由小程序请求选择。上传步骤与行为见 [后台内测登录](../ADMIN_CONSOLE.md#内测登录)。

内测注册另默认开启 `ALLOW_MANUAL_REGISTRATION`：手机号能力不可用时可手填注册，使用可信 OPENID，号码保持未验证，审核／停用、必填资料及学校内号码唯一性仍生效。同步上传 `mcloud` 与重新编译小程序后可使用。正式手机号验证验收前设 `ALLOW_MANUAL_REGISTRATION=false`；该开关独立于后台账号密码登录。详见 [登录与注册说明](WECHAT_LOGIN.md)。
...
运行 `node scripts/check-architecture-deployment.cjs` 会生成字段已经展开的 `.tmp/architecture-validation/indexes-expanded.json`。按目标套餐的索引限制、查询分析与实际命中情况逐项创建。普通索引不能替代数据库范围过滤，也不能优化所有正则包含搜索、`OR`、`!=` 和深分页。
...
所有函数显式设置 `CLOUD_ENV_ID`。它优先于仓库已有 `config.CLOUD_ID`，以免测试部署误连原环境。不要从现有配置推断本次部署目标。首次空库安装还需至少 12 位的 `INIT_ADMIN_PASSWORD`，可设置 `INIT_ADMIN_NAME`；初始化完成后移除初始密码环境变量。旧管理员无需补学校／校区授权即可登录；内测也没有默认 `admin/123456` 自动建号入口。
...
检查成功后，使用控制台/开发者工具“云端安装依赖”部署到确认的环境；API 单独部署 `cloudfunctions/mcloud`。这些本地脚本不会安装、上传或部署。修改源文件后重新生成整个批次，不混用不同批次的函数。订单删除维护的变更同时涉及 API 和订单 worker，不能只更新小程序。

订单扫描包含 `MAIL_ADMIN_DELETED != true`，在真实云环境检查 `worker_expiry`／`worker_overdue` 查询的扫描量、返回量和耗时；事务内仍复查删除标记，避免候选读取后被删除的订单继续生成事件。
...
完成上述准备后，平台管理员进入“管理 → 学校与校区”，维护学校、校区、地点与管理员资料；目录摘要和地点/历史授权详情按需加载。切换到新校区，填写营业时间、价格、履约规则、客服及独立公告，再开启服务。学校注册审核政策由超级管理员在规则页设置，同校各校区共用。

“管理 → 服务范围与营业时间”中的 `enforceBusinessHours`（按营业时间限制发布与接单）默认关闭，旧配置缺少该字段时也关闭，适用于内测全天开放。超级管理员可在当前校区开启，开启后前后端按北京时间的营业区间执行限制；服务总开关关闭时仍暂停新订单，已有订单继续履约。本次调整须同步更新 `mcloud` 云函数与小程序，未执行真实云配置写入。
...
| 管理员登录与操作 | 无授权字段、历史外校授权、旧环境开关 false 均不触发学校／校区门槛；当前请求只访问所属数据；令牌、角色有效，账号状态与限流按配置执行 |
| 微信手机号登录 | AppID、手机号服务权限及额度有效；真实授权后强制补全必填资料；同校号码归属唯一；旧账号补授权；停用／待审／拒绝账号不能靠重登录放行 |
| 手填注册内测 | 原生能力不支持／102 时切换手填，也可直接手填；完成资料后可发布／接单；不伪造手机号验证，关闭服务端开关后恢复授权要求 |
